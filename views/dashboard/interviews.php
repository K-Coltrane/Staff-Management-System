...
<html
  ...
  <head>
    ...
    <title>Staff Management System - Interviews</title>
                ...
                <div class="col-12">
                  <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                      <h5 class="card-title mb-0">All Interviews</h5>
                      <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#scheduleInterviewModal">
                        <i class="icon-base bx bx-plus me-2"></i>Schedule Interview
                      </button>
                    </div>
                    <div class="card-body">
                      <div class="table-responsive">
                        <table class="table table-striped">
                          ...
                          <tbody>
                            ...
                            $interviewsQuery = "SELECT i.*, a.first_name, a.last_name, jp.job_title 
                                              FROM interviews i 
                                              LEFT JOIN applicants a ON i.applicant_id = a.applicant_id 
                                              LEFT JOIN job_postings jp ON i.job_id = jp.job_id 
                                              ORDER BY i.interview_date DESC";
                            $interviewsResult = mysqli_query($conn, $interviewsQuery);
                            
                            if ($interviewsResult && mysqli_num_rows($interviewsResult) > 0):
                                while ($interview = mysqli_fetch_assoc($interviewsResult)):
                            ?>
                            <tr>
                              <td><?php echo $interview['interview_id']; ?></td>
                              <td><?php echo $interview['first_name'] . ' ' . $interview['last_name']; ?></td>
                              <td><?php echo $interview['job_title'] ?? 'N/A'; ?></td>
                              <td><?php echo $interview['interviewer_name']; ?></td>
                              <td><?php echo date('M d, Y H:i', strtotime($interview['interview_date'])); ?></td>
                              <td>
                                <span class="badge bg-<?php echo $interview['interview_type'] == 'phone' ? 'info' : ($interview['interview_type'] == 'video' ? 'primary' : 'success'); ?>">
                                  <?php echo ucfirst($interview['interview_type']); ?>
                                </span>
                              </td>
                              <td>
                                <span class="badge bg-<?php echo $interview['status'] == 'scheduled' ? 'warning' : ($interview['status'] == 'completed' ? 'success' : ($interview['status'] == 'cancelled' ? 'danger' : 'info')); ?>">
                              ...
                              <td>
                                <div class="dropdown">
                                  ...
                                  <ul class="dropdown-menu">
                                    ...
                                    <li><a class="dropdown-item" href="javascript:void(0);">Add Notes</a></li>
